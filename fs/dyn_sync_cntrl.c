...
/*
 * fsync_mutex protects dyn_fsync_active during fb
 * transitions
 */
static DEFINE_MUTEX(fsync_mutex);

bool is_fb_blank __read_mostly = false;
...
static ssize_t dyn_fsync_fb_blank_show(struct kobject *kobj,
		struct kobj_attribute *attr, char *buf)
{
	return sprintf(buf, "fb blank state: %s\n", is_fb_blank ? "BLANK" : "UNBLANK");
}
...
static struct kobj_attribute dyn_fsync_fb_blank_attribute = 
	__ATTR(Dyn_fsync_fb_blank, 0444, dyn_fsync_fb_blank_show, NULL);

static struct attribute *dyn_fsync_active_attrs[] =
	{
		&dyn_fsync_active_attribute.attr,
		&dyn_fsync_version_attribute.attr,
		&dyn_fsync_fb_blank_attribute.attr,
		NULL,
	};
